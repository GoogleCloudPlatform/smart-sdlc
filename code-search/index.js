...
app.post('/process', async (req, res) => {
    let response = "";
    let codeBase = Buffer.from(req.body.codeBase, 'base64').toString('utf8');
    let userQuery = Buffer.from(req.body.userQuery, 'base64').toString('utf8');
    try {
        response = await gcpAiPlatformGemini.callPredict(codeBase, userQuery);
        res.status = 200;
    } catch(e) {
        console.log(e);
